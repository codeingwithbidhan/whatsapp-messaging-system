...
import { fetchAgoraToken } from '../../api/chat.js';
...
export const initiateCall = createAsyncThunk(
  ...
  async ({ participantId, callType, channelId }, { rejectWithValue, getState }) => {
    try {
      ...
      const tokenData = { channelName: channelId, uid: callerId }

      // 1. ব্যাকএন্ড থেকে টোকেন fetch করুন
      const token = await fetchAgoraToken(tokenData);
      ...
      agoraService.sendCallInvitation({
...
const agoraCallSlice = createSlice({
  name: 'agoraCall',
  initialState: {
    // Current active call
    activeCall: null,
